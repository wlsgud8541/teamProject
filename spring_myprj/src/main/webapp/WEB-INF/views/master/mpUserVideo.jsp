		...
		<fieldset>
			<form action="nickModify">
				...
				<table>
					...
					<tr>
						<td>아이디 :</td>
						<td><input type="text" id="cUserId" name="userIdss"
							value="${cUser.userId}" readonly="readonly"></td>
					</tr>
					<tr>
						<td>닉네임 :</td>
						<td><input type="text" name="userNick" id="cUserNick"
		...
		<fieldset>
			...
			<table>
				<c:forEach items="${uVideos}" var="uv">
					<form action="mpVideoDelete">
						<input type="hidden" value="${uv.userId }" name="reUserId">
						<input type="hidden" value="${uv.vNum }" name="vNum">
							...
							<c:when test="${vc4 eq 'https'}">
								<tr>
									<td><img alt="not Found" src="${uv.vImage}" width="150px"
										height="90px"></td>
									<td>${uv.vTitle}</td>
									<td><input type="submit" value="삭제" id="m_deleteVideo"></td>
								</tr>
							</c:when>
							<c:otherwise>
								<tr>
									<td><img alt="not Found"
										src="${pageContext.request.contextPath}/resources/upload/${uv.vImage}"
										width="150px" height="90px" /></td>
									<td>${uv.vTitle}</td>
									<td><input type="submit" value="삭제" id="m_deleteVideo"></td>
								</tr>
							</c:otherwise>
	...
	<div id="m_userComment" style="display: none;">
		<fieldset>
			...
			<c:forEach items="${uComment }" var="uc">
				<form action="mpCommentDelete">
					<input type="hidden" value="${uc.rNum }" name="rNum">
					<table>
						<tr>
							<td>${uc.userNick }</td>
							<td>${uc.rContent }</td>
							<td><input type="submit" value="삭제" id="m_deleteComment"></td>
						</tr>
					</table>
				</form>
			</c:forEach>
