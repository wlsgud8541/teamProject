...
<div id="mps_div">
	...
	<table border="1">
		<c:forEach items="${seUser}" var="se">
			<tr>
				...
				<td>${se.userId}</td>
				<td>${se.userNick}</td>
				<td><a href="choiceUser?choiceUserId=${se.userId}">정보조회</a></td>
			</tr>
		</c:forEach>
	</table>
	</div>
		...
		<fieldset>
			<form action="nickModify">
				...
				<table>
					...
					<tr>
						<td>아이디 :</td>
						<td><input type="text" id="cUserId" name="userId"
							value="${cUser.userId}" readonly="readonly"></td>
					</tr>
					<tr>
						<td>닉네임 :</td>
						<td><input type="text" name="userId" id="cUserNick"
	...
	<div id="m_userComment">
		<fieldset>
			<h3>회원댓글보기</h3>
			<c:forEach items="${uComment}" var="uc">
				<form action="mpCommentDelete">
					<input type="hidden" value="${uc.userNick}" name=choiceNick>
